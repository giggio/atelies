  ...
  describe 'completing payment with manual shipping calculation and product without inventory', ->
    ...
    it 'should have stored a new order on db', (done) ->
      Order.find (err, orders) ->
        ...
        order.totalProductsPrice.should.equal product3.price
        order.totalSaleAmount.should.equal order.totalProductsPrice
        order.deliveryAddress.toJSON().should.be.like user1.deliveryAddress.toJSON()
        p1 = order.items[0]
        p1.product.toString().should.equal product3._id.toString()
        p1.price.should.equal product3.price
        p1.quantity.should.equal 1
        p1.totalPrice.should.equal product3.price
        done()
