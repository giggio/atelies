...
describe 'Home page', ->
  ...
  describe "only authorized stores", ->
    before (done) ->
      cleanDB (error) ->
        ...
        store1.flyerAuthorized = true
        store2 = generator.store.b()
        store2.flyerAuthorized = true
        store1.save()
        store2.save()
        for i in [0..9]
          s = generator.store.a()
          s.name+=i
          s.flyerAuthorized = true
          s.save()
        page.visit done
  ...
  describe.skip "with some stores authorized and some unauthorized flyers", ->
